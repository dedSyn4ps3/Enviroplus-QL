...
# Base Env Class
# Used to instantiate a class object for the standard Enviro HAT
@strawberry.type(
...
# Base Env Class
# Inherits base fields from Env, plus additional readings for the Enviro+ HAT
@strawberry.type(
...
async def get_env_data() -> Env:
    temperature, humidity, pressure = await asyncio.gather(
        get_temperature(),
        get_humidity(),
        get_pressure()
    )
...
async def get_env_data_plus() -> EnvPlus:
